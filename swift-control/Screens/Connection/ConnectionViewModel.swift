        ...
        DeviceManager.shared.selectedDeviceConnected
            .sink { [weak self] in
                self?.alertViewData = AlertViewData(
                    title: Strings.Connection.Alert.Disconnect.title,
                    message: Strings.Connection.Alert.Disconnect.message,
                    actionTitle: Strings.Connection.Alert.Disconnect.action
                ) {
                    Task {
                        await DeviceManager.shared.disconnect()
                    }
